...
describe("getAllLiveRegions", () => {
  ...
  test.each(["status", "log", "alert"])('returns element with [role="%s"]', (role) => {
    element.setAttribute("role", role);

    expect(getAllLiveRegions(root)).toContain(element);
  });

  test.each(["polite", "assertive"])('returns element with [aria-live="%s"]', (ariaLive) => {
  ...
  test.each(["marquee", "timer"])('ignores element with [role="%s"]', (role) => {
    element.setAttribute("role", role);
